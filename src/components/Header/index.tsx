...
export function Header ({ showArrowBack = false }: IHeaderParams): JSX.Element {
  ...
  return (
    <header
      ...
      {
        (location.pathname !== '/' || showArrowBack) && <BiArrowBack className='text-white cursor-pointer md:w-[40px] md:h-[40px]' size={24} onClick={() => { navigate('/'); navigate(0) }}/>

      }
      <img onClick={() => { navigate('/'); navigate(0) }} className='m-auto absolute cursor-pointer' src='assets/images/logo-pokedex.png' alt="logo do pokemon" />
