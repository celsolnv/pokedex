...
export function Home(): JSX.Element {
  ...
  const pokemonQuery = useQuery(
    ['pokemon', currentPage],
    async () => {
      return await fetchPokemon(currentPage);
    },
    { staleTime: 1000 * 60 * 1 }
  ); // 1 minute
  const allPokemonsQuery = useQuery(
    ['pokemons'],
    async () => {
  ...
  async function handleSearch(event: React.FormEvent): Promise<void> {
    ...
    if (allPokemonsQuery.isSuccess && pokemonNameSearch.length >= 3) {
      ...
      const pokemonFilter = allPokemons.filter(({ name }) =>
        name.includes(pokemonNameSearch.toLowerCase())
      );
    ...
    <div>
      ...
      <div>
        <SearchInput
          ...
          setSearchValue={setPokemonNameSearch}
        />
      </div>
