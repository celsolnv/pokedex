...
export function DetailsPokemon(): JSX.Element {
  ...
  const [pokemonsFavorites, setPokemonsFavorites] = useLocalStorage(
    'pokemons_favorite',
    []
  );
  const pokemonQuery = useQuery(
    ['pokemon', namePokemon],
    async () => await getPokemonDetailsByName(String(namePokemon))
  );
  ...
  useEffect(() => {
    if (pokemonQuery.isSuccess && Object.keys(pokemonDetails)) {
      ...
      const checkPokemonFavorite = pokemonsFavorites.find(
        (pokemonName: string) => pokemonName === response.name
      );
  ...
  function handleSwitchFavorite(): void {
    ...
    if (!isFavorite) {
      ...
      const newPokemonsFavorites = pokemonsFavorites.filter(
        (pokemonName: string) => pokemonName !== pokemonDetails.name
      );
